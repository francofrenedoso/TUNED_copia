...
export const CardHeader: FC<ICardHeaderProps> = (props) => {
	const { children, isTuned = false, className, ...rest } = props;

	// TUNED
	const classes = classNames(
		`flex flex-wrap items-center justify-between ${isTuned ? '' : 'gap-4'} px-4 pb-4 [&:first-child]:pt-4`,
		className,
	);
...
export const CardBody: FC<ICardBodyProps> = (props) => {
	const { children, className, tuned = false, ...rest } = props;
	...
	// TUNED
	const classes = classNames('grow', { 'px-4 pb-4 [&:first-child]:pt-4': !tuned }, className);
...
export const CardFooter: FC<ICardFooterProps> = (props) => {
	...
	const classes = classNames(
		'flex flex-wrap items-center justify-between gap-4 pb-4 [&:first-child]:pt-4',
		className,
	);
...
const Card = forwardRef<HTMLDivElement, ICardProps>((props, ref) => {
	const { children, className, bg, rounded = 'auto', ...rest } = props;
	...
	const cardClasses = classNames(
		'flex flex-col',
		// template modificada
		{ [`${bg}`]: bg, 'bg-white dark:bg-zinc-900': !bg },
